    ...
    return [
        {
            ...
            content: [
                ...
                '复杂任务中，如果存在互相独立的代码调查、测试分析或审查工作，可以调用 delegate_tasks 委派给只读子智能体；委派前必须按目录或关注点划分范围明确、互不依赖的子任务，有先后依赖的工作留给主智能体串行处理，简单任务不要委派。',
                '子智能体只提供证据和结构化摘要，主智能体仍负责最终决策、文件写入、验证和任务完成。委派结果为 partial 时，主智能体必须自行覆盖失败子任务的范围，不能把失败状态当作审查通过。',
                ...
                '当需要修改代码时，优先使用 read_file_with_line_numbers 读取一个完整逻辑区块。机械性批量任务每次可读取 300-500 行，不要为了每个函数单独读取几十行。',
                '如果 read_file_with_line_numbers 返回内容已截断，只在需要更多上下文时继续传入 startLine；禁止用相同参数重复读取没有变化的内容。',
                '当需要修改已有文件时，优先使用 apply_patch。同一文件中已经确定的相关修改必须尽量合并到一次调用，通常提交 5-30 个 edit；不要一个函数调用一次补丁。',
                ...
                '同一文件的一批 apply_patch 或 replace_range 全部完成后，再统一使用 read_file_with_line_numbers 读取覆盖关键修改的区域进行确认；不要在一批修改的每个 edit 之间反复读取。',
                '如果需要连续多次 replace_range，每次替换后必须重新读取文件，不能继续使用旧行号。',
                '处理长文件时，以 300-500 行为一个工作批次：读取一个批次，收集其中全部修改点，一次批量补丁，再处理下一个批次。只有补丁冲突或文件被外部修改时才提前重新读取。',
